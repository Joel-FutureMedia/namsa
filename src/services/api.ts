...
export const artistAPI = {
  ...
  uploadMusic: async (musicData: MusicUploadForm): Promise<ArtistWork> => {
    ...
    // Include ArtistId if available (some backends require it for authorization checks)
    try {
      const prof = await api.get('/api/artist/profile').then(r => r.data).catch(() => null);
      const artistIdValue = prof?.ArtistId || prof?.artistId;
      if (artistIdValue) formData.append('ArtistId', String(artistIdValue));
    } catch {}
